...
<div class="app-container">
    ...
    <div class="main-content">
        ...
        <main class="main-content-area">
            ...
            <section id="dashboard" class="section-anchor mb-4"><div class="d-flex justify-content-between align-items-center mb-3"><div><h3 class="fw-bold mb-1">Admin Dashboard</h3><p class="text-muted mb-0">System-wide internship monitoring and verification.</p></div><span class="badge bg-danger">Administrator</span></div><div class="row g-3"><div class="col-md-3"><div class="card stat-card shadow-sm p-3"><small class="text-muted">REGISTERED INTERNS</small><h2 class="mb-0"><?= (int)$intern_count; ?></h2><small class="text-muted">Active, pending, and completed accounts</small></div></div><div class="col-md-3"><div class="card stat-card shadow-sm p-3"><small class="text-muted">SYSTEM-WIDE HOURS</small><h2 class="mb-0"><?= number_format((float)$total_hours, 1); ?></h2><small class="text-muted">Cumulative rendered hours</small></div></div><div class="col-md-3"><div class="card stat-card shadow-sm p-3"><small class="text-muted">PENDING APPROVALS / REQUESTS</small><h2 class="mb-0"><?= (int)$pending_count; ?></h2><small class="text-muted">Logs: <?= (int)$pending_log_count; ?> · Deletions: <?= (int)$request_count; ?></small></div></div><div class="col-md-3"><div class="card stat-card shadow-sm p-3"><small class="text-muted">ACTIVE STATUS</small><h2 class="mb-0"><?= (int)$active_count; ?> <span class="fs-6 text-success">timed in</span></h2><small class="text-muted"><?= (int)$timed_out_count; ?> timed out</small></div></div></div></section>
            ...
            <section id="admin-tools" class="section-anchor card section-card shadow-sm mb-4"><div class="card-header bg-white"><h5 class="mb-0 fw-bold"><i class="bi bi-tools me-2"></i>Administrative Tools & System Settings</h5></div><div class="card-body"><div class="row g-3"><div class="col-md-4"><h6 class="fw-bold">User Role Control</h6><p class="text-muted small">Manage intern account details through the Intern Management edit controls.</p><a href="#intern-management" class="btn btn-sm btn-outline-primary">Manage Users</a></div><div class="col-md-4"><h6 class="fw-bold">Reports & PDF Export</h6><p class="text-muted small">Generate an official master DTR audit report.</p><a href="<?= base_url('ojt/export_pdf'); ?>" target="_blank" class="btn btn-sm btn-outline-danger"><i class="bi bi-file-earmark-pdf me-1"></i>Export PDF</a></div><div class="col-md-4"><h6 class="fw-bold">System Logs & Audit Trail</h6><p class="text-muted small">Review deletion requests and account activity from the management queues.</p><a href="#ojt-management" class="btn btn-sm btn-outline-secondary">Review Activity</a></div></div></div></section>
        </main>
    </div>
</div>
...
<script>
    ...
    setupInternFilters();
    switchAdminModule('dashboard', document.querySelector('.sidebar-menu li.active a'));
